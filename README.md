...
I want to dedicate each agent to a single task instead of letting one agent wear multiple hats. I want my tokens going to the task itself, not to loading everything around it. With subagents, I can talk to just the one scoped to my task and leave the rest out of the conversation. I also don't want to push a one-line fix through four stages, so small tasks skip the pipeline entirely.
